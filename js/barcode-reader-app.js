...
,store = new Vuex.Store({
    ...
    mutations: {
        ...
        setTotalInfo(e){
            var count = 0;
            var price = 0;
            e.savedCodes.forEach(function (item) {
                 count += item.count*1;
                 price += item.price*1;
            });
            e.totalCount = count;
        ...
        saveCode(e, t) {
            var code = e.savedCodes.find(function(val){
               if(val.code == t.code) return true;
               return false;
            });
            if(!code){
              e.savedCodes.unshift(t);
              return;
            }
    ...
    actions: {
        ...
        cleanCodes({commit: e}, t) {
            e("cleanCodes")
            axios
                .post('ajax.php',{type:'clear-stock'})
                .then(response => {
        ...
        setSavedCodes({commit: e},codes){
            e("setSavedCodes",codes);
            e("setTotalInfo");
        }
    },
    getters: {
...
window.app = new Vue({
        ...
        created: function(){
            var self = this;
            axios
                .post('ajax.php',{type:'list'})
                .then(response => {
                    if(response.data.success)
                    {
                        self.$store.dispatch("setSavedCodes",response.data.products);
                    }
                });
        }
    });
